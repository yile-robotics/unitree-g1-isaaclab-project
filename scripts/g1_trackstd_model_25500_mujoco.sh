...
RUN_DIR="${UNITREE_RL_LAB}/logs/rsl_rl/unitree_g1_29dof_velocity/2026-06-04_12-39-20_g1_29dof_trackstd_lin02_yaw015_yawrew2_seed42"
POLICY_SRC="${RUN_DIR}/exported/policy.onnx"
DEPLOY_SRC="${RUN_DIR}/params/deploy.yaml"

MUJOCO_POLICY_DIR="${UNITREE_RL_LAB}/deploy/robots/g1_29dof/config/policy/velocity/model_25500_mujoco"
...
SIM_CONFIG="${SIM_DIR}/config.yaml"
SIM_BIN="${SIM_DIR}/build/unitree_mujoco"

if [[ ! -f "${POLICY_SRC}" ]]; then
  echo "Missing policy.onnx: ${POLICY_SRC}"
  echo "先在 Isaac Sim play.py 里导出这个 checkpoint，或确认 ${RUN_DIR}/exported/policy.onnx 是否存在。"
  exit 1
fi
...
if [[ ! -x "${CTRL_BIN}" ]]; then
  ...
  echo "Build it with:"
  echo "  cd ${CTRL_DIR} && mkdir -p build && cd build && cmake .. && make -j"
  exit 1
fi
...
if [[ ! -x "${SIM_BIN}" ]]; then
  ...
  echo "Build it with:"
  echo "  cd ${SIM_DIR} && mkdir -p build && cd build && cmake .. && make -j"
  exit 1
fi

mkdir -p "${MUJOCO_POLICY_DIR}/exported" "${MUJOCO_POLICY_DIR}/params" "${RUN_DIR}/mujoco_tracking_eval"
cp "${POLICY_SRC}" "${MUJOCO_POLICY_DIR}/exported/policy.onnx"
cp "${DEPLOY_SRC}" "${MUJOCO_POLICY_DIR}/params/deploy.yaml"

# MuJoCo config uses no joystick by default. Switch the command observation to
# the keyboard-driven observation implemented in deploy/robots/g1_29dof/src/State_RLBase.cpp.
...
text = text.replace("    params: {command_name: base_velocity}\n", "    params: {command_name: base_velocity}\n")
...
echo "  cd ${SIM_DIR}"
echo "  MUJOCO_TRACKING_EVAL=1 MUJOCO_TRACKING_OUT=${RUN_DIR}/mujoco_tracking_eval/raw.csv ${SIM_BIN}"
echo
echo "Terminal 2: start G1 controller"
echo "  cd ${CTRL_DIR}"
echo "  MUJOCO_TRACKING_EVAL=1 ${CTRL_BIN} --network lo --auto_fixstand_then_velocity"
echo
echo "After the automatic cases finish, generate the two summary images:"
echo "  cd ${UNITREE_RL_LAB}"
echo "  /home/yile/miniconda3/envs/isaacsim/bin/python scripts/plot_mujoco_tracking_summary.py \\"
echo "    --csv ${RUN_DIR}/mujoco_tracking_eval/raw.csv \\"
echo "    --output ${RUN_DIR}/mujoco_tracking_eval"
echo
