...
class LocalTrajectoryTest(unittest.TestCase):
    ...
    def test_odom_passed_goal_guard_completes_after_sustained_distance_increase(self):
        odom = MutableOdometry()
        follower = LocalTrajectoryFollower(
            LocalFollowerConfig(
                goal_tolerance_m=0.5,
                odom_pass_guard_arm_margin_m=0.2,
                odom_pass_guard_distance_margin_m=0.2,
                odom_pass_guard_confirm_s=0.2,
                blind_yaw_radius_m=0.0,
        ...
        # Approach within tolerance+0.2m without entering the 0.5m success radius.
        odom.pose = Pose2D(1.35, 0.0, 0.0, 1.0)
        approached = follower.update(0.1, np.zeros(3))
        self.assertFalse(approached.reached)
        self.assertTrue(follower.goal_pass_guard_armed)
        self.assertAlmostEqual(follower.best_goal_distance_m, 0.65)

        # Move 0.25m away from the best point and remain there for 0.2s.
        odom.pose = Pose2D(1.10, 0.0, 0.0, 2.0)
        first = follower.update(0.1, np.zeros(3))
        second = follower.update(0.1, np.zeros(3))
        self.assertFalse(first.reached)
        self.assertTrue(second.reached)
        np.testing.assert_allclose(second.command, np.zeros(3))
        self.assertIsNone(second.abort_reason)

    def test_odom_passed_goal_guard_does_not_arm_when_never_close(self):
        odom = MutableOdometry()
        follower = LocalTrajectoryFollower(
            LocalFollowerConfig(
                goal_tolerance_m=0.5,
        ...
        odom.pose = Pose2D(0.5, 0.0, 0.0, 1.0)
        follower.update(0.1, np.zeros(3))
        odom.pose = Pose2D(-0.5, 0.0, 0.0, 2.0)
        outputs = [follower.update(0.1, np.zeros(3)) for _ in range(4)]

        self.assertFalse(follower.goal_pass_guard_armed)
        self.assertTrue(all(not output.reached for output in outputs))
    ...
    def test_default_pure_pursuit_branch_keeps_blind_yaw_behavior(self):
        follower = LocalTrajectoryFollower(
            LocalFollowerConfig(
                goal_tolerance_m=0.1,
                blind_yaw_radius_m=1.5,
                replan_interval_s=10.0,
            )
        )
        follower.start(
            np.array([[0.0, 0.0, 0.0], [0.6, 0.8, 0.0]]),
            np.array([0.6, 0.8]),
        )

        output = follower.update(0.0, np.zeros(3))

        self.assertEqual(follower.config.tracking_controller, "pure_pursuit")
        self.assertAlmostEqual(output.command[2], 0.0)
        self.assertGreater(output.command[0], 0.0)

    def test_kp_branch_uses_bounded_holonomic_command(self):
        follower = LocalTrajectoryFollower(
            LocalFollowerConfig(
                ...
                kp_yaw_deadband_rad=np.deg2rad(10.0),
                kp_max_lateral_speed_m_s=0.12,
                kp_max_yaw_speed_rad_s=0.35,
                replan_interval_s=10.0,
            )
        )
        follower.start(
        ...
        self.assertAlmostEqual(output.command[2], 0.35)
        self.assertGreater(output.command[0], 0.0)
        self.assertAlmostEqual(output.command[1], 0.12)
        np.testing.assert_allclose(output.target_local_xy, [0.3, 0.4])

    def test_kp_branch_tracks_path_tangent_without_blind_yaw(self):
        follower = LocalTrajectoryFollower(
            LocalFollowerConfig(
                tracking_controller="kp",
                goal_tolerance_m=0.1,
        ...
        follower.start(
            np.array([[0.0, 0.0, 0.0], [1.0, 0.05, 0.0]]),
            np.array([1.0, 0.05]),
        )

        output = follower.update(0.0, np.zeros(3))

        self.assertGreater(output.command[2], 0.0)
        self.assertLess(output.command[2], np.deg2rad(10.0))
        self.assertGreater(output.command[0], 0.0)
        self.assertGreater(output.command[1], 0.0)

    def test_kp_branch_preserves_old_reverse_and_lateral_tracking(self):
        odom = MutableOdometry()
        follower = LocalTrajectoryFollower(
            LocalFollowerConfig(
                tracking_controller="kp",
        ...
        self.assertLess(output.command[0], 0.0)
        self.assertAlmostEqual(output.command[1], 0.12)
        self.assertAlmostEqual(output.command[2], 0.35)

    def test_kp_branch_projects_fixed_path_using_odometry(self):
        odom = MutableOdometry()
        follower = LocalTrajectoryFollower(
            LocalFollowerConfig(
                tracking_controller="kp",
        ...
        odom.pose = Pose2D(0.5, 0.2, 0.0, 1.0)

        output = follower.update(0.1, np.zeros(3))

        # Projection is (0.5, 0.0), so the 0.5m lookahead is world (1.0, 0.0)
        # and therefore robot-local (0.5, -0.2).
        np.testing.assert_allclose(output.target_local_xy, [0.5, -0.2])
        self.assertGreater(output.command[0], 0.0)
        self.assertLess(output.command[1], 0.0)
        self.assertAlmostEqual(output.command[2], 0.0)

    def test_rejects_unknown_tracking_controller(self):
        with self.assertRaisesRegex(ValueError, "Tracking controller"):
            LocalFollowerConfig(tracking_controller="unknown").validated()
