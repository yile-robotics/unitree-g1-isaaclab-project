...
class IsaacLocalFourViewCamera:
    ...
    偷用真值坐标。四个传感器仍保留在机器人上；默认 G3 状态机只反复读取 forward，
    通过机器人连续旋转生成四方向全景。``capture_panorama`` 只保留为诊断回退。
    """
...
class IsaacRootOdometryProvider:
    ...
    def get_yaw(self) -> float:
        """Expose the same root-pose yaw to the relative-rotation controller."""

        return float(self.get_pose().yaw)
