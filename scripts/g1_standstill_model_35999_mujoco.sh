...
MUJOCO_POLICY_DIR="${UNITREE_RL_LAB}/deploy/robots/g1_29dof/config/policy/velocity/model_35999_standstill_mujoco"
...
if [[ ! -f "${POLICY_SRC}" ]]; then
  ...
  echo "Run play.py with model_35999.pt first to export the policy."
  exit 1
fi
...
for line in lines:
    ...
    if in_velocity and stripped.startswith("policy_dir:") and not replaced:
        old_policy_dir = stripped.removeprefix("policy_dir:").strip()
        if old_policy_dir != policy_dir:
            out.append(f"    # policy_dir: {old_policy_dir}")
