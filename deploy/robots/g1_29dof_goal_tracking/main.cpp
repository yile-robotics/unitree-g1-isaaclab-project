...
void init_fsm_state()
{
    ...
    FSMState::lowstate = std::make_shared<LowState_t>();
        spdlog::info("Waiting for connection to goal-tracking MuJoCo/robot...");
...
int main(int argc, char** argv)
{
    ...
    const int domain_id = param::config["domain_id"].as<int>(11);
    const std::string network = vm["network"].as<std::string>();

    std::cout
        << " --- Unitree Robotics ---\n"
        << " G1 Independent Goal Tracking Controller\n"
    ...
    if (vm.count("auto_start") || vm.count("auto_fixstand_then_velocity"))
    {
        std::thread([fsm_ptr = fsm.get()] {
            ...
            sleep(5);
            spdlog::info("Auto start: FixStand -> GoalTracking");
            fsm_ptr->requestState("GoalTracking");
        }).detach();
    }
    else
    {
